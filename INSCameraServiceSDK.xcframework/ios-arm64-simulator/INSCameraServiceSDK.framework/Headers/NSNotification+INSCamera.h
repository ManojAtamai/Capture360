...
/*!
 * disconnect camera notification
 * contain bluetoothDisconnectCurrent() & socketDisconnectCurrent()
 */
extern NSString *const INSCameraDisconnectedCameraNotification;
...
/*!
 *  for go3s，
 *  Send interval shots toast
 *  userInfo - {"status": INSIntervalRecInfo}
 */
extern NSString *const INSCameraIntervalRecInfoNotification;

/*!
 *  for go3s，
 *  send interval shots user takeover
 */
extern NSString *const INSCameraUserTakeoverNotification;
...
/*!
 *  for Go3s
 *  userInfo = @{@"usbCardStatus" :  INSUsbCardStatus};
 */
extern NSString *const INSCameraNotificationUsbcardStatusNotification;

/*!
 *  for x4 , camera submode Change
 */
extern NSString *const INSCameraCamSubmodeChangeNotification;

/*!
 *  for x4 , 相机删除文件通知
 * userInfo = @{@"deleteFileOperation": INSNotificationDeleteFileOperation};
 */

extern NSString *const INSDeleteFileOperationNotification;
/*!
 *  文件收藏
 */
extern NSString *const INSFavoriteFileStatusChangeNotification;
/*!
 *  云存储上传状态即将中断
 *   userInfo = @{@"cloudUploadStatus" :  INSCloudUploadState }
 */
extern NSString *const INSCameraNotifyAppCloudUploadInterruptNotification;

extern NSString *const INSFMGStateChangeNotification;
extern NSString *const INSFMGOTAStateChangeNotification;

/*!
 *  for IAC3 , 行车记录仪模式信息
 * userInfo = @{@"dashCamInfo": INSDashCamInfo};
 */
extern NSString *const INSCameraDashcamInfoNotification;

/*!
 *  相机繁忙状态，即有其它设备正在连接
 *   userInfo = nil
 */
extern NSString *const INSCameraBusyNotification;

/*!
 *   虚拟云台是否处于玩法中
 *    userInfo = @{@"ptzMovementStatus": INSPTZMovementStatus}
 */
extern NSString *const INSCameraVirtualPtzMovementStatusNotification;

/*!
 * 相机直播状态切换通知
 */
extern NSString *const INSCameraNotificationLiveOnlineStatusNotification;

/*!
 *   分辨率改变
 *    userInfo = @{@"resolutionUpdate": INSPTZMovementStatus}
 */
extern NSString *const INSCameraResolutionUpdateNotification;

/*
 * Wi-Fi 连接切换信道通知
 */
extern NSString *const INSCameraWifiChannelSwitchResNotification;

/*
 * Wi-Fi RSSI、rx noise、tx rate、信道信息通知
 */
extern NSString *const INSCameraWifiInterferenceStatusNotification;
/*!
 *   通知APP不识别的模式状态变更,目前包含水下模式和长续航模式
 *    userInfo = @{@"notRecognizedStatus"}
 */
extern NSString *const INSCameraNotRecognizedStatusNotification;
/*!
 *     // 通知 APP LivePhoto 切换状态（状态 + 错误码）
 *    userInfo = @{@"livePhotoSwitch"}
 */
extern NSNotificationName const INSCameraLivePhotoSwitchNotification;
/*!
 *     // 通知 微距镜切换状态（状态 ）
 *    userInfo =@{@"macrolensSwitch": @(Bool)};
 */
extern NSNotificationName const INSCameraMacrolensSwitchNotification;

/*
 * Wi-Fi 关闭原因通知
 */
extern NSString *const INSCameraWiFiOffInfoNotification;
